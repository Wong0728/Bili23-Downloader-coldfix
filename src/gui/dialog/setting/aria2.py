...
class DetectDialog(Dialog):
    ...
    def init_UI(self):
        select_lab = wx.StaticText(self, -1, _("请选择 ARIA2 路径"))
        ...
        self.env_path_lab = wx.StaticText(
            self, 
            -1, 
            _("未检测到 ARIA2"), 
            size=self.FromDIP((450, 20)), 
            style=wx.ST_ELLIPSIZE_MIDDLE
        )
        
        self.cwd_chk = wx.RadioButton(self, -1, _("运行目录"))
        self.cwd_path_lab = wx.StaticText(
            self, 
            -1, 
            _("未检测到 ARIA2"), 
            size=self.FromDIP((450, 20)), 
    ...
    def detect_location(self):
        def set_enable(chk_control: wx.Window, lab_control: wx.Window, path: str):
            ...
            lab = path if enable else _("未检测到 ARIA2")
            
            lab_control.SetLabel(lab)
            lab_control.SetToolTip(lab)
        
        aria2_paths = Aria2Env.get_aria2_paths()
        
        env_path, cwd_path = aria2_paths["env_path"], aria2_paths["cwd_path"]
        ...
        if current_path == env_path and env_path:
            self.env_chk.SetValue(True)
        elif current_path == cwd_path and cwd_path:
