...
class Config:
    ...
    class Download:
        ...
        aria2_mode: int = 0  # 0=本地模式, 1=RPC模式
        aria2_thread_count: int = 16
        aria2_path: str = ""
    ...
    @classmethod
    def validate_and_fix_paths(cls):
        """验证并修复配置中的路径问题"""
        need_save = False
        ...
        if download_path:
            if not os.path.exists(download_path):
                Config.Download.path = get_default_download_path()
                need_save = True

        ffmpeg_path = Config.Merge.ffmpeg_path
        if ffmpeg_path and not os.path.isfile(ffmpeg_path):
            Config.Merge.ffmpeg_path = ""
            need_save = True
    ...
    def load_config(cls):
        ...
        cls.validate_and_fix_paths()

    @staticmethod
    def init_path():
        Config.User.directory = os.path.join(os.getcwd(), "data")
