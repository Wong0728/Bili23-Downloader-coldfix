...
from utils.module.aria2_downloader import Aria2Downloader
...
class Downloader:
    ...
    def set_downloader_info(self, downloader_info: List[dict]):
        ...
        # 如果启用了 ARIA2，初始化 ARIA2 下载器
        if Config.Download.enable_aria2:
            if not self.aria2_downloader:
                self.aria2_downloader = Aria2Downloader(self.task_info, self.callback)
            self.aria2_downloader.set_downloader_info(downloader_info)

    def start_download(self):
        # 如果启用了 ARIA2，使用 ARIA2 下载
        if Config.Download.enable_aria2 and self.aria2_downloader:
            try:
                self.aria2_downloader.start_download()
                return
            except Exception as e:
                print(f"ARIA2 下载失败，回退到原生下载: {e}")
                # 回退到原生下载
                self.aria2_downloader = None
    ...
    def stop_download(self):
        ...
        # 如果正在使用 ARIA2，停止 ARIA2 下载
        if self.aria2_downloader:
            self.aria2_downloader.stop_download()
